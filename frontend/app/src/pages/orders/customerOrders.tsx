...
const CustomerOrders = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await fetch("https://fuel-station-j9n7.vercel.app/api/orders/sellers-orders", {
          method: "POST",
  ...
  const handleUpdateStatus = async (orderId: string) => {
    setUpdating(orderId);
    try {
      const response = await fetch(`https://fuel-station-j9n7.vercel.app/api/orders/update/${orderId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      });
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-4 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-4">
                        <div className={`flex items-center gap-2 ${order.status ? 'text-green-600' : 'text-amber-600'
                          }`}>
                          {order.status ? (
                        ...
                        {!order.status && (
                          <button
                            ...
                            className={`px-4 py-2 text-sm rounded-full transition-all ${updating === order._id
